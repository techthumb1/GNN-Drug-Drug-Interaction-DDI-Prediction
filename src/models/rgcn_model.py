...
class RGCN(torch.nn.Module):
    def __init__(self, metadata, num_nodes_dict, hidden_channels=64, out_channels=2, num_layers=2):
        ...
        for node_type in metadata[0]:  # metadata[0] = node types
            self.embeddings[node_type] = Embedding(num_embeddings=num_nodes_dict[node_type], embedding_dim=hidden_channels)
        ...
        for i in range(num_layers):
            self.convs.append(RGCNConv(
                ...
                num_relations=len(metadata[1]),  # metadata[1] = edge types
                num_bases=30
            ))
